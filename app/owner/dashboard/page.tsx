...
export default function OwnerDashboardPage() {
  ...
  const trend7d = [
    { label: "Usuarios", value: new7d.users, total: counts.users, icon: "U" },
    { label: "Medicos", value: new7d.doctors, total: counts.doctors, icon: "M" },
    { label: "Visitas", value: new7d.visits, total: counts.visits, icon: "V" },
    { label: "Feedbacks", value: new7d.feedback, total: counts.feedback, icon: "F" },
  ];
      ...
      <div style={css.wrap}>
          ...
          <div style={css.headerInner}>
            <div style={css.headerLeft}>
              ...
              <p style={css.subtitle}>
                Visao interna de produto, adocao e operacao
              </p>
            </div>
        ...
        <section style={css.midRow}>
          <div style={{ ...css.card, flex: "1 1 420px", padding: 28 }}>
            <SectionHeader
              title="Saude do produto"
              sub="Indicadores principais"
            />
            <div style={css.healthGrid}>
              <HealthTile
                label="Conversao premium"
                value={premiumRate + "%"}
                desc="Usuarios com plano premium"
                fill={premiumRate}
                color="#8B5CF6"
              />
              <HealthTile
                label="Adocao de medicos"
                value={adoptionRate + "%"}
                desc="Vinculos sobre base de medicos"
                fill={adoptionRate}
                color="#10B981"
              />
              <HealthTile
                label="Base do diretorio"
                value={counts.directory.toLocaleString("pt-BR")}
                desc="Medicos disponiveis para busca"
                fill={Math.min(100, Math.round(counts.directory / 10))}
                color="#F59E0B"
              />
              <HealthTile
                label="Interacoes totais"
                value={(counts.feedback + counts.visits).toLocaleString(
                  "pt-BR"
                )}
                desc="Feedbacks + solicitacoes"
                fill={Math.min(
                  100,
                  Math.round((counts.feedback + counts.visits) / 2)
                )}
                color="#3B82F6"
              />
            </div>
          ...
          <div style={{ ...css.card, flex: "0 1 300px", padding: 28 }}>
            <SectionHeader
              ...
              sub="Novos registros por categoria"
            />
            <div
              style={{
                display: "flex",
        ...
        <section>
          <SectionHeader
            ...
            sub="Ultimos registros por categoria"
          />
          <div style={css.tableGrid}>
              ...
              {recentUsers.length === 0 ? (
                ...
                recentUsers.map((u) => (
                  <ActivityRow
                    ...
                    secondary={"Plano: " + (u.plan ?? "-") + " | Role: " + (u.role ?? "-")}
            ...
            <ActivityTable
              title="Medicos recentes"
              count={recentDoctors.length}
            >
              ...
              {recentVisits.length === 0 ? (
                ...
                recentVisits.map((v) => (
                  <ActivityRow
                    ...
                    tag={
                      v.status
                        ? { label: v.status, color: statusColor(v.status) }
                        : undefined
                    }
                  />
                ))
...
function SectionHeader({ title, sub }: { title: string; sub: string }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <h2
        style={{ fontSize: 18, fontWeight: 800, color: "#111827", margin: 0 }}
      >
        {title}
      </h2>
