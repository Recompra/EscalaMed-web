...
type Weekday = "Segunda" | "Terça" | "Quarta" | "Quinta" | "Sexta";
...
export default function HomeClient() {
  ...
  const btnStyle: React.CSSProperties = {
    ...
    border: "1.5px solid rgba(13,17,23,0.12)", cursor: "pointer",
    background: "#fff", fontFamily: "'DM Sans', sans-serif",
    fontSize: 12, fontWeight: 600, color: "#0D1117",
  };

  const deleteBtnStyle: React.CSSProperties = {
    ...btnStyle, background: "#FFF0EE",
    border: "1.5px solid rgba(192,57,43,0.20)",
    color: "#C0392B", marginTop: 6,
  };
  ...
  const selectStyle: React.CSSProperties = {
    ...
    background: "#fff", color: "#0D1117", width: "100%",
  };
  ...
  const filterSpanStyle: React.CSSProperties = {
    ...
    letterSpacing: "0.10em", textTransform: "uppercase", color: "#8A9BB0",
  };
  ...
  async function handleDelete(doctorId: string) {
    ...
    const { data, error } = await supabase
      .from("user_doctors").delete()
      .eq("doctor_id", doctorId).eq("user_id", user.id).select();
    console.log("DELETE user_doctors:", { data, error });
    if (error) { console.error("erro ao remover relação user_doctors:", error); alert("Erro ao excluir."); return; }
  ...
  useEffect(() => {
    ...
    if (!uf || !city || !weekdayNumber || !periodDb) {
      setDoctors([]); setLoading(false); return;
    }
    ...
    const run = async () => {
     ...
     const doctorIds = availabilityIds;
      ...
      const { data, error } = await supabase
        .from("doctors").select("*")
        .eq("state", uf).eq("city", city)
        .in("id", doctorIds).order("created_at", { ascending: false });
      if (error) { console.log("SUPABASE ERROR:", error); setDoctors([]); setErrorMsg("Erro ao buscar médicos"); setLoading(false); return; }
      const rows = data ?? [];
      const unique = Array.from(new Map(rows.map((d: any) => [d.id, d])).values());
      setDoctors(unique);
      setLoading(false);
    };
    run();
...
{status === "failure" && (
  <div style={{ background: "#dc2626", color: "white", padding: 12, borderRadius: 8, marginBottom: 16 }}>
    ...
    <main style={{
      ...
      fontFamily: "'DM Sans', sans-serif",
      padding: "24px", maxWidth: 920, margin: "0 auto",
    }}>
      ...
      {menuOpen && (
        <div onClick={() => setMenuOpen(false)} style={{
          ...
          background: "rgba(13,17,23,0.45)", zIndex: 50,
        }}>
          <div onClick={(e) => e.stopPropagation()} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.40)", marginTop: 2 }}>propagandista</div>
              </div>
              <button type="button" aria-label="Fechar menu" onClick={() => setMenuOpen(false)} style={{
                height: 30, width: 30, borderRadius: "50%", border: "none",
            ...
            {[
              ...
              <a key={item.label} href={item.href} style={{
                ...
                <div style={{
                  ...
                  background: item.label === "EscalaIA"
                ...
                <span style={{
                  ...
                  color: item.label === "EscalaIA" ? "#4ADE80" : "rgba(255,255,255,0.88)",
                }}>{item.label}</span>
                {item.badge && (
                  <span style={{
                    background: item.label === "EscalaIA" ? "rgba(74,222,128,0.20)" : "#1A6B4A",
                    color: item.label === "EscalaIA" ? "#4ADE80" : "#fff",
                    fontSize: 10, fontWeight: 700,
      ...
      <div style={{
        ...
        <button type="button" aria-label="Abrir menu" onClick={() => setMenuOpen(true)} style={{
          height: 38, width: 38, borderRadius: 10,
          border: "1.5px solid rgba(13,17,23,0.10)", background: "#fff",
        ...
        <a href="/admin" aria-label="Novo cadastro" style={{
      ...
      <a href="/ai" style={{ textDecoration: "none" }}>
        <div style={{
          ...
          gap: 16,
          cursor: "pointer",
          boxShadow: "0 4px 24px rgba(26,107,74,0.20)",
          position: "relative",
          overflow: "hidden",
        }}>
          {/* Glow de fundo */}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{
              ...
              IA especializada no universo do rep — análise de Power BI, simulação de acompanhamento e entrevista para GD
            </p>
          </div>

          <div style={{
            color: "rgba(74,222,128,0.60)", fontSize: 18, flexShrink: 0,
          }}>›</div>
        </div>
      </a>
      ...
      {/* SUBTÍTULO */}
      <p style={{
        marginBottom: 20, fontSize: 13, color: "#8A9BB0",
        fontFamily: "'DM Sans', sans-serif",
      }}>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={filterLabelStyle}>
              ...
              <select value={uf} onChange={(e) => onChangeUF(e.target.value as typeof UFS[number])} style={selectStyle}>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={filterLabelStyle}>
              ...
              <select value={weekday} onChange={(e) => setWeekday(e.target.value as Weekday)} style={selectStyle}>
                {WEEKDAYS.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
            </label>
            <label style={filterLabelStyle}>
              <span style={filterSpanStyle}>Período</span>
              <select value={period} onChange={(e) => setPeriod(e.target.value as Period)} style={selectStyle}>
      ...
      {/* RESULTADO ATIVO */}
      <div style={{
        marginTop: 16, border: "1px solid rgba(13,17,23,0.08)",
        borderRadius: 10, padding: "11px 16px", background: "#fff",
        fontSize: 13, color: "#4A5568", fontFamily: "'DM Sans', sans-serif",
      }}>• {title}</div>
      ...
      <section style={{ marginTop: 12 }}>
        {doctors.length === 0 ? (
          <div style={{
            ...
            background: "#FFF3DC", borderRadius: 12,
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {doctors.map((d) => (
              <div key={d.id} style={{
                padding: 16, border: "1px solid rgba(13,17,23,0.08)",
                ...
                <div style={{ display: "grid", gap: 5 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#8A9BB0" }}>{d.specialty} · {d.phone}</div>
                  <div style={{ fontSize: 12, color: "#8A9BB0" }}>{d.clinic} — {d.address}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => router.push(`/admin?id=${d.id}`)} style={btnStyle}>Editar</button>
                  <button type="button" onClick={() => handleDelete(d.id)} style={deleteBtnStyle}>Excluir</button>
                </div>
              </div>
            ))}
          </div>
